...
export class ConsoleFormat {
	...
	constructor(opts = {}) {
		...
		if (typeof this.opts.inspectOptions === 'undefined' && this.opts.breakLength) {
	...
	message(info, chr, color) {
		...
		if (this.opts.showTimestamp && info.timestamp) {
			...
			timestamp = this.opts.showTimestamp?.onlyTime ? time : `${year}-${month}-${day} ${time}`;
		}
		...
		if (splat) {
			if (sum(values(sizes)) <= this.opts.breakLength) {
				splat = inspect(splat, { ...this.opts.inspectOptions, breakLength: Infinity });
		...
		if (this.opts.showTimestamp?.right) {
			...
			const padSize = this.opts.breakLength - sum(values(sizes)) - size(timestamp) + 1;
	...
	splat(info) {
		...
		if (Object.keys(stripped).length > 0) {
			...
			return stripped;
		} else if (!info.stack && size(splat) === 1 && typeof splat[0] !== 'undefined') {
			return splat[0];
		}
	}
